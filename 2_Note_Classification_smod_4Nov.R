...
# Set dir
#train_Dir <- "C:/Users/suyas/OneDrive/Desktop/WBS_songs/Sholicola songs segments/manually_classified_training_data"
test_Dir <- "C:/Users/suyas/OneDrive/Desktop/WBS_songs/Sholicola songs segments/BWRR-2022"
Dir <- "C:/Users/suyas/OneDrive/Desktop/WBS_songs/Sholicola songs segments/Temp" #create this dir initially if needed - copy all training data into this

# Training and test data file names
train_data <- "Subsampled_Training_Data.txt"
test_data <- "Combine-notes-BWRR-2022_Note-segment.txt"

setwd(test_Dir)
# copy all the files from test to the temp directory
list.of.files <- list.files(test_Dir)
file.copy(list.of.files, Dir)
...
train_data_complete <- data.table::fread(file_path_train)
#train_data_complete$`Note Type` <- NA
train_data_complete$`Correlation` <- NA
test_data_complete <- data.table::fread(file_path_test)
...
# Set Parameters
lowlim <- 0.7 # Threshold for new note type
highlim <- 0.95 # Threshold to stop look and assign note type

Test_N <- 50 # Test for a subset
...
write.table(train_data_complete, train_data,
